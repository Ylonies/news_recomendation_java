...
class ThreeDNewsParserTest {
  ...
  void getArticle() {
    ...
    assertAll(
        () -> assertEquals("В России перестанут работать мобильное приложение и сервисы Keenetic с 1 марта, но выход есть", article.getName(), "Assert article name"),
        () -> assertEquals("С 1 марта 2025 года в России прекращают работу мобильное приложение", article.getDescription().substring(0, 67), "Assert article description"),
        () -> assertEquals("16.01.2025 [17:17]", article.getDate(), "Assert article date")
    );
  }
}